...
class Agent:
    def __init__(self, room_num, gamma, actor_lr, critic_lr, tau, capacity, batch_size, save_path,
        ...
        self.s_dim = room_num * 3 + 2  # 每个房间每一步的状态维度有3个：Ti, Tm和Ti_ref，整个园区额外有2个状态: regD功率和To
        ...
        # 采用目标网络可以避免Q值的更新发生震荡
        self.actor = Actor(self.s_dim*2, 128, self.a_dim)
        self.actor.to(device)
        self.actor_target = Actor(self.s_dim*2, 128, self.a_dim)
        self.actor_target.to(device)
        self.critic_1 = Critic(self.s_dim*2 + self.a_dim, 256, self.a_dim)
        self.critic_1.to(device)
        self.critic_2 = Critic(self.s_dim*2 + self.a_dim, 240, self.a_dim)
        self.critic_2.to(device)
        self.critic_1_target = Critic(self.s_dim*2 + self.a_dim, 256, self.a_dim)
        self.critic_1_target.to(device)
        self.critic_2_target = Critic(self.s_dim*2 + self.a_dim, 240, self.a_dim)
...
class AreaNet:
    ...
    def power_allocation(self, r_reg, ti_ref_list, t_o, s_last):
        # 中央控制器对园区内房间进行功率分配，并更新温度，同时负责进行强化学习数据的存储
        p_reg = 1/2 * self.power_limit * (1 + 1/2 * r_reg)
        state_now = []
        for room_i in range(self.room_n):
        ...
        state_now.append(t_o/50)
        state_now.append(r_reg)    # 状态量，包含每个房间的各类温度与regD信号，归一化处理
        s0 = state_now
        ...
        for room_i in range(self.room_n):
            if abs(self.room_list[room_i].ti - ti_ref_list[room_i]) > 1:
                reward_now -= abs(self.room_list[room_i].ti - ti_ref_list[room_i])**2 / 20
            else:
                reward_now += 0.2
            electricity_power += self.q_list[room_i]/COP
        if abs(electricity_power - p_reg) < self.power_limit/50:
            reward_now += 0.1
        else:
            reward_now = reward_now - 0.01
        # 奖赏值定义，取决于RegD信号与温度
        if not self.done:
            d1 = 1
        else:
...
time_length = 60 * 12 * 15  # 总时长，设为12h，每4秒为1个时段
Ti_set_1 = 25  #房间1设定温度
Ti_set_2 = 24  #房间2设定温度
...
if TRAIN_MODE:  # 训练模式下，直接开启训练并生成训练模型
    ...
    for episode in range(EPOCH):  # 训练过程
        ...
        for step in range(MAX_STEP):
            ...
            '''
            ratio_reg = RegD_list[(step - 1) % repeat_len]
            P_reg = base_power + 0.5 * base_power * ratio_reg
            total_step += 1
            if step == MAX_STEP - 1:
                MyNet.done = True
            Q_room, r, state_last = MyNet.power_allocation(ratio_reg, Ti_train_set, To_list[int((step - 1) * dt)], state_last)
            episode_reward += r
            MyNet.Agent.learn()
            print("learn:{}/{}".format(step + 1, MAX_STEP))
            if total_step % 200000 == 199999 and total_step < 1000000:
                MyNet.Agent.variance /= 10  # 每执行200000step减小噪声方差
                ''''''
...
state_last_1.append(RegD_list[0])
for t in range(time_length):
    ratio_reg = RegD_list[(t - 1) % repeat_len]
    Q_reg = base_power / COP + 0.5 * base_power / COP * ratio_reg  # 规定RegD信号
    Q_reg_list.append(Q_reg)
    Q_list, r, state_last_1 = MyNet.power_allocation(ratio_reg, Ti_ref_list, To_list[int((t - 1) * dt) + start_day * 24], state_last_1)
    # 根据TD3算功率
